...
// getNewTimerNode 함수 추가
function getNewTimerNode(currentNode) {
  const newTimerNode = {
    id: `timer-${currentNode.id}`, // 새로운 ID 생성
    type: "timer", // 타이머 노드 유형 지정
    data: { label: "타이머 노드" }, // 타이머 노드에 해당하는 데이터
    position: { x: currentNode.position.x + 100, y: currentNode.position.y }, // 기존 노드의 위치를 기준으로 조금 이동시킴
  };
  return newTimerNode;
}
  ...
  const stepActionHandle = (item) => {
    ...
    if (connectedEdge.length > 1) {
      if (
        ...
        // Edges actions or update
        const standAloneEdge = _.cloneDeep(connectedEdge[0]);
        standAloneEdge.target = getNewTimerNode(currentNode).id; // getNewTimerNode 사용
        standAloneEdge.source = prevNode.id;
        standAloneEdge.type = connectedEdge[0]?.data?.condition
          ? EdgeTypes.custom
          : EdgeTypes.bridge;
        ...
        setEdges((edges) => {
          ...
          const newlyBindedTimerEdge = addNewEdge(
            getNewTimerNode(currentNode).id,
            getNewNode.id,
            "default"
          );
          const newlyFloatEdge = addNewEdge(
